...
func _draw() -> void:
	...
	# Distance is derived from level data, not a literal tied to the old finish
	# at x=916 (852 = 916-64), so relocating finish (walker-jumpman-yuan Zone 3) still reports 0-100%.
	var route_length: float = float(game.level.finish[0]) - float(game.level.spawn[0])
	var progress: float = clampf((game.player.position.x-game.level.spawn[0])/route_length, 0, 1)
