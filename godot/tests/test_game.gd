...
func run() -> void:
	...
	var route = Route.new()
	var route_ticks := 0
	# Cap raised from the starter's 900 to fit the longer walker-jumpman-yuan
	# route (1312px spawn-to-finish vs the original 852px, plus 3 more jumps);
	# a generous multiple of the observed completion tick count recorded in
	# TEST-REPORT.md, not a loosened pass/fail assertion.
