...
func _draw() -> void:
	# "Crate-Bot": original geometric redesign, no imported art. Chassis/feet
	# stay inside the 18x28 collider (local x -9..9, y -28..0); only the
	# antenna deliberately overshoots the box top by ~5px as a cosmetic-only
	# silhouette break (see CHANGE-BRIEF.md failure case 1 / TEST-REPORT.md).
	var ink := Color("25354a")
	var panel := Color("3f8f6d")
	var stripe_a := Color("ef875f")
	var visor := Color("fff9e9")
	var stride := sin(float(tick) * 0.7) * 2.0 if is_on_floor() and absf(velocity.x) > 8 else 0.0

	draw_rect(Rect2(-9, -27, 18, 24), ink)
	draw_rect(Rect2(-7, -25, 14, 20), panel)

	draw_rect(Rect2(-2, -33, 4, 3), stripe_a)
	draw_rect(Rect2(-1, -31, 2, 4), ink)

	# Pupil uses "panel", not "ink": an ink pupil at the visor's leading edge
	# sat flush against the ink chassis border and was invisible in practice
	# (found by pixel-sampling evidence/screens/state-facing-*.png; see
	# TEST-REPORT.md inspect-and-revise log).
	var visor_x := 0.0 if facing > 0 else -8.0
	draw_rect(Rect2(visor_x, -23, 8, 3), visor)
	draw_rect(Rect2(visor_x + 5.0 if facing > 0 else visor_x, -23, 3, 3), panel)

	for i in range(5):
		var sx := -9.0 + float(i) * 3.6
		draw_rect(Rect2(sx, -12, 3.6, 4), stripe_a if i % 2 == 0 else ink)

	draw_rect(Rect2(-8, -4, 6, 4 + stride), ink)
	draw_rect(Rect2(2, -4, 6, 4 - stride), ink)
