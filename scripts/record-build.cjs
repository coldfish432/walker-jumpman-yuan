...
const record = {
  project:'walker-jumpman-yuan', build_id:hash(JSON.stringify(sorted)),
  created_at:new Date().toISOString(),
  engine:'4.7.2.stable.official.ed1daf0bf', renderer:'Compatibility',
  scope:'First Steps slice extended with Zone 3 (Stacked Landings) and the Crate-Bot character redesign; starter is nikbearbrown/walker-jumpman',
  ...
  screenshots:['01-menu','02-failure','03-jump','05-zone3-landing','04-complete','state-facing-right','state-facing-left','state-jumping','state-on-narrow-landing'].map(n=>({file:'evidence/screens/'+n+'.png',sha256:hash(fs.readFileSync(path.join(evidence,'screens',n+'.png')))}))
};
